...
if [ ! -f patch-integration.py ] || [ ! -f patch-player-inactive.py ] || [ ! -f patch-player-name.py ] || [ ! -f patch-player-board-shop.py ] || [ ! -f patch-flex-controls.py ] || [ ! -f patch-host-layout.py ] || [ ! -f patch-giftcard-prizes.py ] || [ ! -f giftcard-prizes.js ] || [ ! -f giftcard-prizes-client.js ] || [ ! -f giftcard-stores.example.json ]; then
...
PATCH_HASH="$(cat patch-integration.py patch-player-inactive.py patch-player-name.py patch-player-board-shop.py patch-flex-controls.py patch-host-layout.py patch-giftcard-prizes.py giftcard-prizes.js giftcard-prizes-client.js giftcard-stores.example.json | sha256sum | awk '{print $1}')"
...
if [ ! -f runtime/server.js ] || [ "$CURRENT_STAMP" != "$BUILD_STAMP" ]; then
  ...
  python3 patch-host-layout.py runtime/server.js
